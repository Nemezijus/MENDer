...
from shared_schemas.ensemble_configs import BaggingEnsembleConfig
...
def _make_default_tree_pipeline(
    *,
    ...
    feature_strategy = make_features(
        run_cfg.features,
        seed=features_seed,
        # model_cfg is used by feature selection logic; for default-tree case we pass None-ish by
        # reusing a minimal placeholder decision based on eval config (feature strategy may not need it).
        # If your make_features requires a concrete ModelConfig, we can refactor this later.
...
def build_bagging_ensemble(
    ...
    *,
    ...
    if cfg.base_estimator is not None:
        ...
        base_est = _make_default_tree_pipeline(
            run_cfg=run_cfg,
            rngm=rngm,
            stream=f"{stream}/base",
            kind=expected_kind,
...
__all__ = ["build_bagging_ensemble", "fit_bagging_ensemble"]
