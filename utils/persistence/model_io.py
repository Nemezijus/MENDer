"""
Model artifact save/load utilities (business logic layer).
...
# If LZ4 is available, we’ll use it; otherwise fall back to gzip/zlib (built-in).
# joblib accepts either ("codec", level) or just an int level for zlib.
def _joblib_compress_arg():
    if _HAS_LZ4:
        return ("lz4", 3)          # fast & small if available
    return 3                        # zlib level 3 (balanced)
