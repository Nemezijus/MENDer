...
"""Normalization helpers for metrics payloads.

These helpers translate outputs from Engine metric computation into
JSON-friendly response shapes expected by the API models.

Historically these lived in the backend service layer; Segment 12 moves
them into BL reporting so scripts and use-cases can share the same logic.
"""

from typing import Any, Dict, List, Optional, Tuple

from engine.reporting.common.json_safety import safe_float_list, safe_float_scalar


def normalize_confusion(confusion_payload: Optional[Dict[str, Any]]) -> Tuple[List[Any], List[List[Any]], List[Any], Any, Any, Any]:
    """Normalize confusion payload from metric computation into JSON-friendly pieces.

    Returns
    -------
    (labels_out, matrix_out, per_class, overall, macro_avg, weighted_avg)
    """
