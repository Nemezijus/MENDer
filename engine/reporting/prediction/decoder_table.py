...
"""Decoder per-sample table builders (classification only)."""
...
from engine.reporting.common.json_safety import ReportError, add_report_error, error_row, error_row_from_errors
        ...
        if isinstance(label, bool):
            ...
            if re.fullmatch(r"-?\d+\.0+", s):
                try:
                    ...
                    add_report_error(errors, where="reporting.prediction.decoder_table.proba_cols", exc=e, context={"label": str(label)})
    except Exception:
        s = str(label).strip()
...
def build_decoder_output_table(
    *,
    ...
    try:
        ...
        ds_arr = to_2d_array(decision_scores) if decision_scores is not None else None
        pr_arr = to_2d_array(proba) if proba is not None else None
        mg_arr = to_1d_array(margin) if margin is not None else None

        cls_arr = to_1d_array(classes) if classes is not None else None
        ...
        if ds_arr is not None:
            ds_arr = slice_first_n(ds_arr, n_rows)
        if pr_arr is not None:
            pr_arr = slice_first_n(pr_arr, n_rows)
        if mg_arr is not None:
            mg_arr = slice_first_n(mg_arr, n_rows)
            ...
            try:
                if np is not None:
                    ...
                    if ds_is_binary_scalar:
                        ds_1d_arr = to_1d_array(decision_scores)
                else:
                    ds_is_binary_scalar = (
                ...
                if ds_is_binary_scalar and ds_1d_arr is not None:
                    ...
                    if score_cols:
                        ...
                        try:
                            ...
                            add_report_error(errors, where="reporting.prediction.decoder_table.score_cols", exc=e, context={"label": str(label)})
        ...
        if errors:
            rows.append(error_row_from_errors(errors))

        return rows
    except Exception as e:
        return [error_row(where="reporting.prediction.build_decoder_output_table", exc=e)]
