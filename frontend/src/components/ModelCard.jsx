    ...
    <Stack gap="sm">
      ...
      {algo === 'logreg' && (
        <Stack gap={6} ml="xs">
          ...
          <Select label="solver" data={['lbfgs', 'liblinear', 'newton-cg', 'sag', 'saga'].map(s => ({ value: s, label: s }))} value={logreg.solver} onChange={(v) => setLogReg('solver', v)} />
          <NumberInput label="max_iter" value={logreg.max_iter} onChange={(v) => setLogReg('max_iter', v)} step={50} min={1} />
          <Select label="class_weight" data={[{ value: 'none', label: 'none' }, { value: 'balanced', label: 'balanced' }]} value={logreg.class_weight ?? 'none'} onChange={(v) => setLogReg('class_weight', v === 'none' ? null : 'balanced')} />
      ...
      {algo === 'svm' && (
        <Stack gap={6} ml="xs">
          ...
          {(svm.kernel === 'poly' || svm.kernel === 'sigmoid') && <NumberInput label="coef0" value={svm.coef0} onChange={(v) => setSVM('coef0', v)} step={0.1} />}
          ...
          <Select label="class_weight" data={[{ value: 'none', label: 'none' }, { value: 'balanced', label: 'balanced' }]} value={svm.class_weight ?? 'none'} onChange={(v) => setSVM('class_weight', v === 'none' ? null : 'balanced')} />
          <NumberInput label="max_iter (-1 = unlimited)" value={svm.max_iter} onChange={(v) => setSVM('max_iter', v)} step={100} />
          <Select label="decision_function_shape" data={[{ value: 'ovr', label: 'ovr' }, { value: 'ovo', label: 'ovo' }]} value={svm.decision_function_shape} onChange={(v) => setSVM('decision_function_shape', v)} />
      ...
      {algo === 'tree' && (
        <Stack gap={6} ml="xs">
          <Select label="criterion" data={['gini', 'entropy', 'log_loss'].map(c => ({ value: c, label: c }))} value={tree.criterion} onChange={(v) => setTree('criterion', v)} />
          <Select label="splitter" data={['best', 'random'].map(s => ({ value: s, label: s }))} value={tree.splitter} onChange={(v) => setTree('splitter', v)} />
          ...
          />
          {tree.max_features_mode === 'int' && <NumberInput label="max_features (int)" value={tree.max_features_value} onChange={(v) => setTree('max_features_value', v)} min={1} />}
          {tree.max_features_mode === 'float' && <NumberInput label="max_features (0–1 float)" value={tree.max_features_value} onChange={(v) => setTree('max_features_value', v)} min={0} max={1} step={0.05} />}
          ...
          <Select label="class_weight" data={[{ value: 'none', label: 'none' }, { value: 'balanced', label: 'balanced' }]} value={tree.class_weight ?? 'none'} onChange={(v) => setTree('class_weight', v === 'none' ? null : 'balanced')} />
      ...
      {algo === 'forest' && (
        <Stack gap={6} ml="xs">
          ...
          />
          {forest.max_features_mode === 'int' && <NumberInput label="max_features (int)" value={forest.max_features_value} onChange={(v) => setForest('max_features_value', v)} min={1} />}
          {forest.max_features_mode === 'float' && <NumberInput label="max_features (0–1 float)" value={forest.max_features_value} onChange={(v) => setForest('max_features_value', v)} min={0} max={1} step={0.05} />}
          ...
          <Select label="class_weight" data={[{ value: 'none', label: 'none' }, { value: 'balanced', label: 'balanced' }, { value: 'balanced_subsample', label: 'balanced_subsample' }]} value={forest.class_weight ?? 'none'} onChange={(v) => setForest('class_weight', v === 'none' ? null : v)} />
      ...
      {algo === 'knn' && (
        <Stack gap={6} ml="xs">
          <NumberInput label="n_neighbors (k)" value={knn.n_neighbors} onChange={(v) => setKNN('n_neighbors', v)} min={1} step={1} />
          <Select label="weights" data={['uniform', 'distance'].map(w => ({ value: w, label: w }))} value={knn.weights} onChange={(v) => setKNN('weights', v)} />
          <Select label="algorithm" data={['auto', 'ball_tree', 'kd_tree', 'brute'].map(a => ({ value: a, label: a }))} value={knn.algorithm} onChange={(v) => setKNN('algorithm', v)} />
          <NumberInput label="leaf_size" value={knn.leaf_size} onChange={(v) => setKNN('leaf_size', v)} min={1} />
          <NumberInput label="p (1=Manhattan, 2=Euclidean)" value={knn.p} onChange={(v) => setKNN('p', v)} min={1} step={1} />
          <Select
            label="metric"
            data={[
              'minkowski', 'euclidean', 'manhattan', 'chebyshev', 'hamming', 'canberra',
            ].map(m => ({ value: m, label: m }))}
            value={knn.metric}
            onChange={(v) => setKNN('metric', v)}
          />
          <NumberInput label="n_jobs" value={knn.n_jobs} onChange={(v) => setKNN('n_jobs', v)} step={1} />
          <Text size="xs" c="dimmed">Tip: KNN strongly benefits from feature scaling.</Text>
        </Stack>
      )}
